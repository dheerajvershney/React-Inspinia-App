import React, {Component} from 'react';
let Blogs = React.createClass({
    render: function () {
        return (
            <div className="wrapper wrapper-content  animated fadeInDown blog">
                <div className="row">
                    <div className="col-lg-4">
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        One morning, when Gregor Samsa
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Adam Novak</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 28th Oct 2015</span>
                                </div>
                                <p>
                                    English.Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for 'lorem ipsum' will uncover many web
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-primary btn-xs" type="button">Model</button>
                                        <button className="btn btn-white btn-xs" type="button">Publishing</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 56 comments </div>
                                            <i className="fa fa-eye"> </i> 144 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        It showed a lady fitted out with
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Mike Smith</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 11th May 2015</span>
                                </div>
                                <p>
                                    The bedding was hardly able to cover it and seemed ready to slide off any moment.His many legs, pitifully thin compared with the size of.
                                </p>
                                <p>
                                    No one rejects, dislikes, or avoids pleasure itself, because it is pleasure, but because those who do not know how to pursue pleasure rationally encounter consequences
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-primary btn-xs" type="button">New</button>
                                        <button className="btn btn-white btn-xs" type="button">Model</button>
                                        <button className="btn btn-white btn-xs" type="button">Publishing</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 36 comments </div>
                                            <i className="fa fa-eye"> </i> 100 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        The bedding was hardly able to cover
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Anthony Dvorak</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 11th Dec 2014</span>
                                </div>
                                <p>
                                    A collection of textile samples lay spread out on the table - Samsa was a travelling salesman.
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-white btn-xs" type="button">Publishing</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 11 comments </div>
                                            <i className="fa fa-eye"> </i> 46 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="col-lg-4">
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        Junk MTV quiz graced by fox whelps.
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>alex Trebek</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 07 Apr 2015</span>
                                </div>
                                <p>
                                    Alex Trebek's fun TV quiz game. Woven silk pyjamas exchanged for blue quartz. Brawny gods just zebra, and my wolves quack! Blowzy red vixens fight for a quick.
                                </p>
                                <p>
                                    On the other hand, we denounce with righteous indignation and dislike men who are so beguiled and demoralized by the.
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-white btn-xs" type="button">Model</button>
                                        <button className="btn btn-white btn-xs" type="button">Publishing</button>
                                        <button className="btn btn-white btn-xs" type="button">Modern</button>
                                        <button className="btn btn-white btn-xs" type="button">Premium</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 23 comments </div>
                                            <i className="fa fa-eye"> </i> 34 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        Two driven jocks help fax my big
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Monica Jackson</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 14th Oct 2015</span>
                                </div>
                                <p>
                                    Flummoxed by job, kvetching W.zaps Iraq.Cozy sphinx waves quart jug of bad milk.A very bad quack might jinx zippy fowls.Few quips galvanized
                                </p>
                                <p>
                                    It wasn't a dream. His room, a proper human room although a little too small, lay peacefully between its four familiar walls.
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-white btn-xs" type="button">Modern</button>
                                        <button className="btn btn-white btn-xs" type="button">Model</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 32 comments </div>
                                            <i className="fa fa-eye"> </i> 10 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        I feel the presence of the Almighty
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>John Smith</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 04 Dec 2015</span>
                                </div>
                                <p>
                                    I hear the buzz of the little world among the stalks, and grow familiar with the countless indescribable forms of the insects and flies, then I feel the presence of the Almighty, who formed us in his own image, and the breath
                                    Gregor then turned to look out the window at the dull weather.
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-white btn-xs" type="button">Publishing</button>
                                        <button className="btn btn-white btn-xs" type="button">Premium</button>

                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 22 comments </div>
                                            <i className="fa fa-eye"> </i> 17 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                    <div className="col-lg-4">
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        The European languages are members
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Anthony Pits</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 04 Jan 2015</span>
                                </div>
                                <p>
                                    The languages only differ in their grammar, their pronunciation and their most common words.Everyone realizes why a new common language would be desirable: one could refuse to pay expensive translators.
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-primary btn-xs" type="button">Publishing</button>
                                        <button className="btn btn-white btn-xs" type="button">Model</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 54 comments </div>
                                            <i className="fa fa-eye"> </i> 52 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        The new common language
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Adam Novak</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 14th Oct 2015</span>
                                </div>
                                <p>
                                    English.Many desktop publishing packages and web page editors now use Lorem Ipsum as their default model text, and a search for 'lorem ipsum' will uncover many web
                                </p>
                                <p>
                                    The bedding was hardly able to cover it and seemed ready to slide off any moment.His many legs, pitifully thin compared with the size of the rest of him, waved about helplessly as he
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-white btn-xs" type="button">Publishing</button>
                                        <button className="btn btn-white btn-xs" type="button">Model</button>
                                        <button className="btn btn-white btn-xs" type="button">Premium</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 47 comments </div>
                                            <i className="fa fa-eye"> </i> 138 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                        <div className="ibox">
                            <div className="ibox-content">
                                <a href="article.html" className="btn-link">
                                    <h2>
                                        Their pronunciation and their most
                                    </h2>
                                </a>
                                <div className="small m-b-xs">
                                    <strong>Mike Johnson</strong> <span className="text-muted"><i className="fa fa-clock-o"></i> 11th Oct 2015</span>
                                </div>
                                <p>
                                    To take a trivial example, which of us ever undertakes laborious physical exercise, except to obtain some advantage from it?But who has any right to find fault with a man who chooses
                                </p>
                                <div className="row">
                                    <div className="col-md-6">
                                        <h5>Tags: </h5>
                                        <button className="btn btn-primary btn-xs" type="button">Publishing</button>
                                        <button className="btn btn-white btn-xs" type="button">Model</button>
                                    </div>
                                    <div className="col-md-6">
                                        <div className="small text-right">
                                            <h5>Stats: </h5>
                                            <div> <i className="fa fa-comments-o"> </i> 54 comments </div>
                                            <i className="fa fa-eye"> </i> 52 views
                                        </div>
                                    </div>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>


            </div>);
    }
});
export default Blogs;

